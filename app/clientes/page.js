  ...
  async function buscarCep(cep) {
    ...
    try {
      ...
      if (!d.erro) {
        ...
        showMsg('CEP encontrado! Endereco preenchido automaticamente.','ok')
      } else {
        showMsg('CEP nao encontrado','err')
      }
    ...
    <AppLayout>
      <div style={{padding:24,maxWidth:1200}}>
        <div style={{marginBottom:24,display:'flex',alignItems:'flex-start',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
          ...
          <div style={{background:'rgba(6,182,212,0.15)',border:'1px solid rgba(6,182,212,0.3)',borderRadius:12,padding:'10px 18px',textAlign:'center'}}>
            ...
            <div style={{fontSize:11,color:'#3D5070'}}>clientes</div>
          </div>
        </div>
        ...
        <div style={{background:'#0F1729',border:'1px solid rgba(96,165,250,0.07)',borderRadius:16,padding:24,marginBottom:24}}>
          ...
          <form onSubmit={save}>
            ...
            <div style={{background:'rgba(6,182,212,0.05)',border:'1px solid rgba(6,182,212,0.15)',borderRadius:12,padding:16,marginBottom:16}}>
              <div style={{fontSize:11,fontWeight:700,color:'#67E8F9',letterSpacing:'1px',textTransform:'uppercase',marginBottom:12}}>Endereco — Digite o CEP para preencher automaticamente</div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:14}}>
                <div>
                  <label style={{display:'block',fontSize:10,fontWeight:700,color:'#3D5070',letterSpacing:'0.8px',textTransform:'uppercase',marginBottom:6}}>
                    CEP {cepLoading && <span style={{color:'#67E8F9'}}>Buscando...</span>}
                  </label>
                  <input placeholder="00000-000" value={form.cep} onChange={e=>handleCep(e.target.value)} style={{...S,borderColor:cepLoading?'#06B6D4':'rgba(96,165,250,0.13)'}} maxLength={9}/>
                </div>
                {[
              ...
              <div style={{marginTop:14}}>
                ...
                <input placeholder="Proximo ao mercado X, portao azul, casa amarela..." value={form.referencia} onChange={e=>f('referencia',e.target.value)} style={S}/>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea placeholder="Informacoes adicionais sobre o cliente..." value={form.observacoes} onChange={e=>f('observacoes',e.target.value)} style={{...S,minHeight:70,resize:'vertical'}}/>
            </div>
            ...
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead>
                <tr style={{background:'rgba(15,23,41,0.6)'}}>
                  {['Nome / Empresa','Telefone','Cidade / UF','Referencia','Acoes'].map(h=>(
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={5} style={{padding:32,textAlign:'center',color:'#3D5070'}}>Carregando...</td></tr>
                ) : filtered.length===0 ? (
                  <tr><td colSpan={5} style={{padding:32,textAlign:'center',color:'#3D5070'}}>Nenhum cliente encontrado</td></tr>
                ) : filtered.map(c=>(
      ...
      {modal&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.75)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(3px)'}} onClick={()=>setModal(null)}>
          <div style={{background:'#0F1729',border:'1px solid rgba(96,165,250,0.2)',borderRadius:18,padding:28,width:520,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
              <span style={{fontSize:16,fontWeight:800,color:'#EEF2FF'}}>{modal.nome}</span>
              <button onClick={()=>setModal(null)} style={{background:'none',border:'none',color:'#3D5070',fontSize:22,cursor:'pointer',lineHeight:1}}>x</button>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
              ...
              <div style={{gridColumn:'1/-1'}}>
                <div style={{fontSize:10,fontWeight:700,color:'#67E8F9',letterSpacing:'0.8px',textTransform:'uppercase',marginBottom:4}}>Ponto de Referencia</div>
                <div style={{fontSize:13,color:'#EEF2FF',background:'rgba(6,182,212,0.08)',padding:'10px 14px',borderRadius:8,border:'1px solid rgba(6,182,212,0.15)'}}>{modal.referencia||'--'}</div>
              </div>
