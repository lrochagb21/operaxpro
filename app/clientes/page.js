        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.75)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(3px)'}} onClick={()=>setModal(null)}>
          <div style={{background:'#0F1729',border:'1px solid rgba(96,165,250,0.2)',borderRadius:18,padding:28,width:540,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
              {[
                ['CPF / CNPJ',modal.documento,false],
                ['Telefone Principal',modal.telefone,true],
                ['Telefone Secundario',modal.telefone2,true],
                ['CEP',modal.cep,false],
                ['Logradouro',modal.logradouro ? `${modal.logradouro} ${modal.numero||''}` : null,false],
                ['Bairro',modal.bairro,false],
                ['Cidade / Estado',`${modal.cidade||''} ${modal.estado?'/ '+modal.estado:''}`,false],
              ].map(([l,v,isPhone])=>v?(
                <div key={l}>
                  ...
                  {isPhone?(
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                      <span style={{fontSize:13,color:'#EEF2FF'}}>{v}</span>
                      <button onClick={()=>window.open('https://wa.me/55'+v.replace(/\D/g,''),'_blank')} title="Abrir no WhatsApp" style={{background:'none',border:'none',cursor:'pointer',padding:0,display:'flex',alignItems:'center'}}>
