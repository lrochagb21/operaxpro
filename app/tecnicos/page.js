...
export default function Tecnicos() {
  ...
  async function save(e) {
    e.preventDefault()
    if (!form.nome.trim()) { showMsg('Informe o nome do técnico','err'); return }
    ...
    if (editId) {
      ...
      else { showMsg('✅ Técnico atualizado com sucesso!','ok'); resetForm() }
    } else {
      const { error } = await supabase.from('usuarios').insert({...form, perfil:'tecnico', empresa_id: me?.empresa_id || 1})
      if (error) { showMsg('Erro: '+error.message,'err') }
      else { showMsg('✅ Técnico cadastrado com sucesso!','ok'); resetForm() }
    }
    setSaving(false)
    load()
  }
...
export default function Tecnicos() {
  ...
  const filtered = list.filter(t => t.nome?.toLowerCase().includes(search.toLowerCase()) || t.email?.toLowerCase().includes(search.toLowerCase()) || t.especialidade?.toLowerCase().includes(search.toLowerCase()))
    ...
    <AppLayout>
      <div style={{padding:24,maxWidth:1200}}>
        ...
        <div style={{marginBottom:24,display:'flex',alignItems:'flex-start',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
          <div>
            <h1 style={{fontSize:22,fontWeight:800,color:'#EEF2FF',letterSpacing:'-0.3px'}}>👷 Técnicos</h1>
            <p style={{fontSize:12,color:'#3D5070',marginTop:3}}>Cadastro e gestão de técnicos de campo</p>
          </div>
          <div style={{background:'rgba(26,86,219,0.15)',border:'1px solid rgba(26,86,219,0.3)',borderRadius:12,padding:'10px 18px',textAlign:'center'}}>
            ...
            <div style={{fontSize:11,color:'#3D5070'}}>técnicos cadastrados</div>
          </div>
        </div>
        ...
        <div style={{background:'#0F1729',border:'1px solid rgba(96,165,250,0.07)',borderRadius:16,padding:24,marginBottom:24}}>
          <div style={{fontSize:14,fontWeight:800,color:'#EEF2FF',marginBottom:18,paddingBottom:14,borderBottom:'1px solid rgba(96,165,250,0.07)',display:'flex',alignItems:'center',gap:8}}>
            {editId ? '✏️ Editar Técnico' : '➕ Cadastrar Novo Técnico'}
            {editId && <button onClick={resetForm} style={{marginLeft:'auto',padding:'4px 12px',borderRadius:8,background:'rgba(239,68,68,.12)',border:'1px solid rgba(239,68,68,.25)',color:'#FCA5A5',fontSize:12,fontWeight:600,cursor:'pointer'}}>Cancelar edição</button>}
          </div>
          <form onSubmit={save}>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:14,marginBottom:16}}>
              {[
                {k:'nome',l:'Nome Completo *',ph:'João Silva',req:true},
                {k:'telefone',l:'Telefone / WhatsApp *',ph:'(11) 99999-9999',req:true},
                {k:'email',l:'E-mail',ph:'joao@email.com',type:'email'},
                {k:'endereco',l:'Endereço',ph:'Rua, número - Bairro'},
                {k:'cidade',l:'Cidade / Estado',ph:'São Paulo / SP'},
              ].map(({k,l,ph,req,type}) => (
                <div key={k}>
                  ...
                  <input type={type||'text'} required={req} placeholder={ph} value={form[k]} onChange={e=>f(k,e.target.value)} style={S}
                    onFocus={e=>e.target.style.borderColor='#1A56DB'} onBlur={e=>e.target.style.borderColor='rgba(96,165,250,0.13)'}/>
                </div>
              ))}
              <div>
            ...
            <div style={{display:'flex',justifyContent:'flex-end',gap:10}}>
              ...
              <button type="submit" disabled={saving} style={{padding:'10px 24px',borderRadius:10,background:'#1A56DB',border:'none',color:'#fff',fontSize:13,fontWeight:800,cursor:'pointer',opacity:saving?0.6:1,boxShadow:'0 4px 14px rgba(26,86,219,.35)'}}>
                {saving ? 'Salvando...' : editId ? '💾 Atualizar Técnico' : '💾 Salvar Técnico'}
              </button>
            </div>
          </form>
        </div>
        ...
        <div style={{background:'#0F1729',border:'1px solid rgba(96,165,250,0.07)',borderRadius:16,overflow:'hidden'}}>
          <div style={{padding:'16px 20px',borderBottom:'1px solid rgba(96,165,250,0.07)',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
            <span style={{fontSize:14,fontWeight:800,color:'#EEF2FF'}}>Lista de Técnicos ({filtered.length})</span>
            <input type="text" placeholder="🔍 Buscar por nome, email ou especialidade..." value={search} onChange={e=>setSearch(e.target.value)}
              style={{...S,width:280,padding:'8px 14px',fontSize:13}} onFocus={e=>e.target.style.borderColor='#1A56DB'} onBlur={e=>e.target.style.borderColor='rgba(96,165,250,0.13)'}/>
          </div>
          <div style={{overflowX:'auto'}}>
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead>
                <tr style={{background:'rgba(15,23,41,0.6)'}}>
                  {['Nome','Telefone','E-mail','Especialidade','Cidade','Status','Ações'].map(h=>(
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={7} style={{padding:32,textAlign:'center',color:'#3D5070',fontSize:13}}>Carregando...</td></tr>
                ) : filtered.length === 0 ? (
                  <tr><td colSpan={7} style={{padding:32,textAlign:'center',color:'#3D5070',fontSize:13}}>
                    {search ? `Nenhum resultado para "${search}"` : 'Nenhum técnico cadastrado ainda'}
                  </td></tr>
                ) : filtered.map(t => {
                  ...
                  return (
                    <tr key={t.id} onMouseEnter={e=>e.currentTarget.style.background='rgba(96,165,250,0.03)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                      <td style={{padding:'13px 16px',borderBottom:'1px solid rgba(96,165,250,0.05)'}}>
                        <div style={{fontWeight:700,color:'#EEF2FF',fontSize:14}}>{t.nome}</div>
                      </td>
                      ...
                      <td style={{padding:'13px 16px',borderBottom:'1px solid rgba(96,165,250,0.05)'}}>
                        <div style={{display:'flex',gap:6}}>
                          <button onClick={()=>editTec(t)} style={{padding:'5px 12px',borderRadius:8,background:'rgba(26,86,219,.15)',border:'1px solid rgba(26,86,219,.3)',color:'#60A5FA',fontSize:12,fontWeight:600,cursor:'pointer'}}>✏️ Editar</button>
                          <button onClick={()=>del(t.id)} style={{padding:'5px 12px',borderRadius:8,background:'rgba(239,68,68,.12)',border:'1px solid rgba(239,68,68,.25)',color:'#FCA5A5',fontSize:12,fontWeight:600,cursor:'pointer'}}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  )
