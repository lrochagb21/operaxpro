      ...
      {modal&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.8)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(4px)'}} onClick={()=>setModal(null)}>
          <div style={{background:'#0F1729',border:'1px solid rgba(96,165,250,0.2)',borderRadius:20,padding:26,width:500,maxHeight:'90vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            ...
            {(()=>{
                const _msg=['*OperaxPro – Lembrete de Atendimento*','','*Cliente:* '+(modal.clientes?.nome||''),'*Data:* '+modal.data?.split('-').reverse().join('/'),'*Horario:* '+modal.hora_inicio?.slice(0,5)+(modal.hora_fim?' - '+modal.hora_fim.slice(0,5):''),'*Tecnico:* '+(modal.usuarios?.nome||'A definir'),modal.tipo_servico?'*Servico:* '+modal.tipo_servico:'','*Endereco:* '+(modal.clientes?.logradouro||modal.endereco||''),'','_Enviado pelo OperaxPro_'].filter(Boolean).join('\n')
                const _send=tel=>window.open('https://wa.me/55'+tel.replace(/\D/g,'')+'?text='+encodeURIComponent(_msg),'_blank')
                const _telCli=modal.clientes?.telefone||modal.telefone||''
                const _telTec=modal.usuarios?.telefone||''
                const _manual=()=>{const n=prompt('Digite o numero com DDD:');if(n)_send(n)}
                return(<div style={{display:'flex',gap:8,marginBottom:8}}>
                  {_telCli&&<button onClick={()=>_send(_telCli)} style={{padding:'10px 14px',borderRadius:10,background:'rgba(37,211,102,0.15)',border:'1px solid rgba(37,211,102,0.3)',color:'#25D366',fontSize:12,fontWeight:700,cursor:'pointer',flex:1,textAlign:'center'}}>📱 Cliente</button>}
                  {_telTec&&<button onClick={()=>_send(_telTec)} style={{padding:'10px 14px',borderRadius:10,background:'rgba(37,211,102,0.15)',border:'1px solid rgba(37,211,102,0.3)',color:'#25D366',fontSize:12,fontWeight:700,cursor:'pointer',flex:1,textAlign:'center'}}>🔧 Técnico</button>}
                  <button onClick={_manual} style={{padding:'10px 14px',borderRadius:10,background:'rgba(37,211,102,0.08)',border:'1px solid rgba(37,211,102,0.2)',color:'#25D366',fontSize:12,fontWeight:700,cursor:'pointer',flex:1,textAlign:'center'}}>✏️ Manual</button>
                </div>)
              })()}
